...
QPoint RectangularSelectionTool::mapToImage(const QPoint& p)
{
...
RectangularSelectionTool::RectangularSelectionTool(UIManager* ui, QWidget* parent)
...
void RectangularSelectionTool::zoomIn()
{
    applyZoom((1.111));
}

void RectangularSelectionTool::zoomOut()
{
    applyZoom((0.9));
}

void RectangularSelectionTool::resetZoom()
{
    zoomPercentage = 100.0;
    applyZoom(1);
}

void RectangularSelectionTool::applyZoom(float zoomAmount)
{
...
void RectangularSelectionTool::keyPressEvent(QKeyEvent* event)
{
    ...
    {
        emit rectangularSelectionToolDisabled();
 
        emit brushEnabled();
    }
}

void RectangularSelectionTool::keyReleaseEvent(QKeyEvent* event)
{
...
void RectangularSelectionTool::mouseMoveEvent(QMouseEvent* event)
{
...
void RectangularSelectionTool::mouseReleaseEvent(QMouseEvent* event)
{
    if (event->button() == Qt::LeftButton)
    {
        if (isPanning) {
...
void RectangularSelectionTool::clearSelectionOverlay()
{
    overlay.fill(Qt::transparent);
    update();
}
...
void RectangularSelectionTool::updateSelectionOverlay()
{
    ...
    if (startPoint != QPoint(0, 0) && hoverPoint != QPoint(0, 0) && isDrawing)
    {
        if (isDrawingSquare)
        {
            ...
            QRect rectangle = QRect(startPoint, hoverPoint);
        ...
        if (isDrawingInPlace)
        {
            ...
            QRect rectangle = QRect(initialPoint, temporaryReleasePoint);
            painter.drawRect(rectangle);
        }
